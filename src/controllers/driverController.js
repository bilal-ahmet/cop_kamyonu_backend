...
const getDrivers = async (req, res) => {
    try {
        ...
        const query = includeInactive
            ...
            : 'SELECT * FROM drivers WHERE is_active = TRUE ORDER BY full_name';
...
const createDriver = async (req, res) => {
    try {
        ...
        if (license_no) {
            const dup = await pool.query('SELECT id FROM drivers WHERE license_no = $1', [license_no]);
            if (dup.rowCount > 0) return res.status(409).json({ error: 'Bu ehliyet numarası zaten kayıtlı' });
        }

        const result = await pool.query(
            'INSERT INTO drivers (full_name, license_no, phone, birth_date) VALUES ($1, $2, $3, $4) RETURNING *',
            [full_name, license_no || null, phone || null, birth_date || null]
        );
...
const updateDriver = async (req, res) => {
    try {
        ...
        const exists = await pool.query('SELECT id FROM drivers WHERE id = $1', [id]);
        ...
        if (license_no !== undefined) {
            const dup = await pool.query('SELECT id FROM drivers WHERE license_no = $1 AND id != $2', [license_no, id]);
            if (dup.rowCount > 0) return res.status(409).json({ error: 'Bu ehliyet numarası zaten kayıtlı' });
